document.addEventListener('DOMContentLoaded', async function() {
    ...
    ui.manager = {
        ...
        renderMusicCard(item, index = 0, ranking = null) {
            const img = (item.images && item.images.length > 0) ? item.images.url : 'https://via.placeholder.com/150';
            const sub = item.type === 'artist' ? ((item.genres && item.genres.length > 0) ? item.genres : t('artists')) : (item.artists?.map(a => a.name).join(', ') || t('albums'));
