document.addEventListener('DOMContentLoaded', async function() {
    ...
    api.manager = {
        // ... (funções _request, _spotifyRequest, etc. sem alterações) ...
    ...
    // ... (AUTH MANAGER e outras funções sem alterações) ...
    auth.manager = {
    ...
    ui.manager = {
        ...
        updateForAuthState() {
            ...
            if (u) {
                ...
                // Constrói o HTML final, garantindo que as insígnias fiquem abaixo do nome
                this.dom.userInfo.innerHTML = `<span id="userName">${u.name}</span>${badgesHTML}`;
            }
        },
        
        // ... (resto das funções do UI MANAGER sem alterações) ...
        switchContent(id) { 
    ...
    // ... (Render HomePage e AlbumView sem alterações)...
    formatDuration = (ms) => { const minutes = Math.floor(ms / 60000); const seconds = ((ms % 60000) / 1000).toFixed(0); return `${minutes}:${seconds < 10 ? '0' : ''}${seconds}`; };
    ...
    async function renderArtistView(artistId, artistName) {
        ...
        // ADICIONA A ORIGEM DO ARTISTA AQUI
    ...
    // ... (O resto do script, como handlers e init, sem alterações) ...
